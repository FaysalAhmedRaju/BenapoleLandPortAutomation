...
<table>
    ...
    @foreach($manifestDetails as $key => $manifestDetail)
        <tr>
            ...
            <td>{{ $manifestDetail->NAME }} <br> VAT NO.{{ $manifestDetail->BIN }}</td>
...
<P class="paragraph">
    <b>
        @foreach($checkApproveVarification as $key => $check)
            Assaignment Status :&nbsp;&nbsp;<b class="status">{{ $check->assessmet_status }}</b>
        @endforeach
    </b>
</P>
