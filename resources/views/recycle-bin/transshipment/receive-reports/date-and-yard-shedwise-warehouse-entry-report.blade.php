...
<table width="100%" class="dataTable"  border="0">
    ...
    <tr>
        ...
        <th style=" font-size: 12px"><nobr>Yard/Shed</nobr></th>
    ...
    @foreach($todaysWareHouseEntry as $key => $wareHouseEntry)
        <tr>
            ...
            <td>{{ $wareHouseEntry->recive_comment }}</td>
            <td>{{ $wareHouseEntry->receive_datetime }}</td>
            <td>{{ $wareHouseEntry->labor_unload }}</td> @php($sumLabourUnload+= $wareHouseEntry->labor_unload)
            <td>{{ $wareHouseEntry->labor_package }}</td>
            <td>{{ $wareHouseEntry->equip_unload }}</td> @php($sumEquipmentUnload += $wareHouseEntry->equip_unload )
            <td>{{ $wareHouseEntry->equip_name }}</td>
            <td>{{ $wareHouseEntry->equipment_package }}</td>
