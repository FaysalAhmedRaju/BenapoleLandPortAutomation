...
<html>
<head>
    <title>Date And Manifest Wise WareHouse Local Transport Delivery Report</title>
    <style>
        table {
...
<p class="center">
    ...
    {{date('d-m-Y',strtotime($date))}}'s WareHouse Local Transport Delivery Report (Manifest)
</p>
...
<table class="posting_data">
    ...
    @foreach($data as $key => $getData)
        <tr>
            ...
            <td>{{ (number_format($getData->receive_packages, 2, '.', '')   -   number_format($getData->labor_equ_package, 2, '.', '')) == 0 ? null : (number_format($getData->receive_packages, 2, '.', '')   -   number_format($getData->labor_equ_package, 2, '.', '')) }}</td>
            ...
            <td>{{  (number_format($getData->receive_weights, 2, '.', '')  -  number_format($getData->labor_equ_weight, 2, '.', '')) == 0 ? null : (number_format($getData->receive_weights, 2, '.', '')  -  number_format($getData->labor_equ_weight, 2, '.', '')) }}</td>
