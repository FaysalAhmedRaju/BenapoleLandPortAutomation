...
<head>
	...
	<body>
		...
		<table>
			<caption style="padding-bottom: 10px;"><b><u>Delivery Request Requisition Details</u><b></caption>
			<thead>
			<tr>
				<th>S/L</th>
				<th>Manifest No.</th>
			...
			<tbody>  @php($i=0)
			@foreach($deliveryRequisition as $key => $dataReq)
				<tr>
					<td> {{ ++$i }}</td>
					<td>{{ $dataReq->manifest }}</td>
					<td>{{ $dataReq->carpenter_packages }}</td>
					<td>{{ $dataReq->carpenter_repair_packages }}</td>

					<td>{{ $dataReq->approximate_delivery_date }}</td>
					<td>{{ $dataReq->approximate_labour_load }}</td>
					<td>{{ $dataReq->approximate_equipment_load }}</td>
					{{--<td>{{ $indianTruck->labor_unload }}</td>@php($indTotalLabourUnload+= $indianTruck->labor_unload)
					<td>{{ $indianTruck->equip_name }}</td>
					<td>{{ $indianTruck->equip_unload }}</td>@php($indTotalEquipmentUnload+= $indianTruck->equip_unload)--}}
					<td>{{ $dataReq->transport_truck}}</td>
					<td>{{ $dataReq->transport_van}}</td>
					<td>{{ $dataReq->local_weighment }}</td>
				</tr>
			@endforeach
			</tbody>
