...
<html>
<head>
    <title>Yearly Posting Entry</title>
    <style>
        html {
            margin: 5px 5px 0;
        }
...
<table  class="dataTable" style="width: 100%;">
    ...
    @foreach($data as $key => $value)
        <tr>
            <td>{{ ++$i }}</td>
            <td>{{date('F Y',strtotime($value->manifest_created_time))}}</td>
            <td>{{ $value->manifest_count }}@php($total_entry+=$value->manifest_count)</td>
