...
<html>
<head>
    <title>Monthly Truck Entry Exit</title>
    <style>
        html {
            margin: 5px 5px 0;
        }
...
<table width="100%;" border="0">
    <tr>
        ...
        <td style="width: 50%; text-align: center">
            ...
            {{$month}}'s Truck Entry-Exit Report
        </td>
...
<table  class="dataTable" style="width: 100%;">
    ...
    @foreach($data as $key => $v)
        <tr>
            <td>{{ ++$i }}</td>
            <td>{{date('d-m-Y',strtotime($v->truckentry_datetime))}}</td>
            <td>{{ $v->entry_truck }}@php($total_entry+=$v->entry_truck)</td>
            <td>{{ $v->exit_truck }}@php($total_exit+=$v->exit_truck)</td>
            <td>{{ $v->entry_truck + $v->exit_truck }}</td>
