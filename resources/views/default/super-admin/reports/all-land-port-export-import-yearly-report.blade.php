        ...
        @if($data)
            @foreach($data as $key=>$value)
                <tr>
                    ...
                    <td>{{number_format($value->import_2017_18,2)}}@php($total_import1+=$value->import_2017_18)</td>
                    <td></td>
                     <td>{{number_format($value->import_2017_18,2) }}@php($total1+=($value->import_2017_18))</td>
                    <td>{{number_format($value->cnF_value_2017_18)}}@php($total_cnf_value+=($value->cnF_value_2017_18))</td>
                     <td>{{$value->export_2018_19}}@php($total_export2+=$value->export_2018_19)</td>
                    <td>{{$value->import_2018_19 + $value->export_2018_19}}@php($total2+=($value->import_2018_19 + $value->export_2018_19))</td>
                    <td></td>
                    <td></td>
                </tr>
       @endforeach
...
<tr>
    ...
    <tr>
        <th></th>
        <th>Total:</th>
        <th>{{number_format($total_import1,2)}}</th>
        <th>{{$total_export1 ?  number_format($total_export1,2) :''}}</th>
        <th>{{number_format($total1,2)}}</th>
        <th>{{number_format($total_cnf_value,0)}}</th>
        <th>{{$total_import2 ? $total_import2:''}}</th>
        <th>{{$total_export2?$total_export2:''}}</th>
        <th>{{$total2?$total2:''}}</th>
