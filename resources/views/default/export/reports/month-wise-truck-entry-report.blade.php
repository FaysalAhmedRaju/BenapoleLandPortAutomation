...
<html>
<head>
    <title>Export Truck Entry Report</title>
    <style>
        html {
            margin: 5px 12px 0;
        }
...
<table width="100%;"  class="dataTable">
    <tr>
        ...
        <td style="width: 60%; text-align:center">
            ...
            <span style="font-size: 19px;">Month Wise Truck Entry Report</span> <br>
            <span style="font-size: 19px;"> From Date: {{date('d-m-Y',strtotime($from_date))}} to Date: {{date('d-m-Y',strtotime($to_date))}}</span>
...
<table>
    ...
    @if(isset($expenditure)  && count($expenditure) > 0)
        @foreach($expenditure as $key => $ex)
            <tr>
                <td width="60">{{ ++$key }}</td>
                {{--@if($key==0){--}} <td>{{$ex->type_name}} - {{$ex->truck_no}}</td>
                <td>{{date('d-m-Y',strtotime($ex->entry_datetime))}}</td>
                <td>{{ $ex->created_by}}</td>
                <td>{{ $ex->haltage_day}}</td>
                <td>{{ $ex->entrance_fee}}</td>
