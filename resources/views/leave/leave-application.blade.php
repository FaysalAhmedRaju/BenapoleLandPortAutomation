    ...
    <div class="col-md-8">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                ...
                {!! Form::open(['route' => ['leave-application-create'],'method' => 'POST','class' => ' ','files' => true]) !!}
                        ...
                        <select class="form-control" title="" name="leave_id">
                            @if($leaveList)
                                ...
                                @foreach($leaveList as $k=>$leave)
                                    <option {{old('leave_id') == $leave->id?'selected':''}} value="{{$leave->id}}">{{$leave->name}}</option>
                                @endforeach
                            @endif
                        </select>
                    </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-control" title="" name="employee_id">
                                ...
                                @foreach($employeeList as $k=>$emp)
                                    <option {{old('leave_id') == $emp->id?'selected':''}} value="{{$emp->id}}">{{$emp->name}}</option>
                                @endforeach
