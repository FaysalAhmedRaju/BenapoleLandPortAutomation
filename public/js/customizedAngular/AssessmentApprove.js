angular.module('AssessmentApproveApp',['angularUtils.directives.dirPagination', 'customServiceModule'])
    .controller('AssessmentApproveCtrl', function($scope,$http,$filter,manifestService,amountToTextService){
        ...
        $scope.$watch('searchText', function(){
            $scope.searchText = manifestService.addYearWithManifest($scope.searchText, $scope.keyboardFlag, $scope.selectionField);
        });
        ...
        $scope.selectField = function () {
            ...
            if ($scope.selectionField == 'manifestNo'){
                $scope.placeHolder = 'Enter Manifest No';
                $scope.serachField = false;

            }else if ($scope.selectionField == 'importerNo'){
                $scope.placeHolder = 'Enter Importer No';
                $scope.serachField = false;


            }else if ($scope.selectionField == 'billNo'){
        ...
        $scope.manifestImporterBillSearch = function (searchText) {

            var data = {
                searchBy : $scope.selectionField,
                searchKey : searchText
            }
            console.log(data);
            $http.post("/api/getmanifestImporterBillSearch", data)
                .then(function (data) {

                    if(data.status == 203) {
                        $scope.ManifestNotFoundError = data.data.noPermission;
                        return;
                    }
                    ...
                    if(data.data.length>0) {
                        ...
                        $scope.ManifestNotFoundError = "Assessment is not Verified.";
                    }

                }).catch(function() {

                $scope.ManifestNotFoundError = "Something went wrong";
        ...
        $scope.details = function(manifest) {
            ...
            $http.post("/api/DetailsOfAssessmentApprove", data)
                .then(function (data) {
                    ...
                    $scope.assessmentApproveForm = true;
                }).catch(function() {
...
//===============================================approve=======================
        $scope.approve = function() {
            var data = {
                manifestNo : $scope.manifestForCaption ,
                approve_comment : $scope.approve_comment
            }

            console.log(data)

            $http.post("/api/applyApprove", data)
                .then(function(data){

                    //console.log(data);
                    $scope.savingSuccess = "Manifest No: "+ $scope.manifestForCaption  +" is Approved.";
                    $scope.approve_comment = null;
                    $scope.show = true;
        ...
        $scope.rejectmanifest = function () {
            ...
            //console.log(data);
            $http.post("/api/applyrejectmanifest", data)
                .then(function(data){
                    console.log(data);
                    // $scope.savingSuccess = "Manifest No:"+ manifestNo +" is Rejected.";
                    $scope.savingError = "Manifest No:"+ $scope.manifestForCaption +" is Rejected.";
                    $scope.approve_comment = null;
                    $scope.show = true;
                }).catch(function(){
                    $scope.savingError = "Something Went Wrong.";
        ...
        $scope.clear = function(){
            ...
            $scope.ManifestNotFoundError = false;
            ...
            $scope.assessmentApproveForm = false;
        }
    ...
    }).filter('approveFilter', function () {
        return function (val) {
            var approve;
            if(val == 0){
               return approve='Not Approved';
            } else if(val == 1) {
