            ...
            $http.post("/assessment/api/assessment/partial/all-partial-details", data)//it also returns all charge and document details
                ...
                .then(function (data) {
                   // console.log('sdfd');
                    ...
                    $scope.chargeable_weight =90// data.data.remaining_weight_pack//Math.ceil(data.data.remaining_weight_package[0].balance_weight / 1000);
                    ...
                    //item_wise_charge--------------------
                    $scope.item_wise_yard_charge = data.data.item_wise_yard_charge;
                    $scope.item_wise_shed_charge = data.data.item_wise_shed_charge;
                    $scope.TotalWarehouseCharge = 0;
                    ...
                    if ($scope.WareHouseRentDay > 0 && $scope.WareHouseRentDay <= 21) {
                        ...
                        if($scope.item_wise_shed_charge) {

                            angular.forEach($scope.item_wise_shed_charge, function (v, k) {
                                if (v.dangerous == '1') {
                                    $scope.TotalWarehouseCharge += Math.ceil((v.item_quantity * $scope.firstSlabDay * 2 * v.first_slab));
                                }
                                else {
                        ...
                        //Total Charge warehouse charge=== YARD
                        if($scope.item_wise_yard_charge) {
                        ...
                        //Total Charge warehouse charge=== SHED
                        if($scope.item_wise_shed_charge) {
