angular.module('incomeApp', ['angularUtils.directives.dirPagination', 'customServiceModule'])
    .controller('incomeCtrl', function ($scope, $http, enterKeyService,manifestService,$filter) {
        ...
        $http.get("/accounts/income/api/get-all-income-sub-head")//get subhead for autocomplete
            .then(function (data) {

                $scope.allExpenditureSubHeadData = data.data;
                console.log( $scope.allExpenditureSubHeadData);
      ...
      //  $scope.showExpenseLimitAlert();
        ...
        $scope.changeProgressbarClass=function (currentPercent,id) {
            if(currentPercent>=0 && currentPercent<80) {
        ...
        $scope.headWiseMonthlyYearlyDataShow = function (head_id) {

            $http.get("/accounts/expenditure/api/head-wise-monthly-yearly-data/" + head_id)
                .then(function (r) {
                    $scope.monthly_head_limit = r.data[0].monthly_head_limit;
                    $scope.yearly_head_limit = r.data[0].yearly_head_limit;
                    $scope.current_expense_Monthly_head = r.data[0].current_expense_Monthly_head;
                    $scope.current_expense_yearly_head = r.data[0].current_expense_yearly_head;


                    $scope.yearlyLimitProgressBar = Math.floor (($scope.current_expense_yearly_head/$scope.yearly_head_limit)*100);
                    $scope.monthlyLimitProgressBar = Math.floor (($scope.current_expense_Monthly_head/$scope.monthly_head_limit)*100);


                    $scope.changeHeadWiseProgressbarClass($scope.yearlyLimitProgressBar,'yearLimitHeadWise');
                    $scope.changeHeadWiseProgressbarClass($scope.monthlyLimitProgressBar,'monthLimitHeadWise');
        ...
        $scope.changeHeadWiseProgressbarClass=function (currentPercent,id) {
        ...
        $scope.getVoucherDetails = function (voucher) {
            ...
            $http.get("/accounts/income/api/get-income-voucher-details/" + voucher)
                .then(function (r) {
        ...
        $scope.getAllExpenditures = function (voucher_no) {
            ...
            $http.get("/accounts/income/api/get-all-income/" + voucher_no)
                .then(function (r) {
                    console.log(r);
                    if (r.data.length <= 0) {
        ...
        $scope.saveExpenditure = function (form) {
            ...
            if (form.$valid) {
                ...
                console.log(data);
                $http.post("/accounts/income/api/save-income", data)
                    .then(function (data) {
                        console.log(data);
                        if (data.status == 201) {
        ...
        $http.get("/accounts/income/api/get-all-income-sub-head")
            .then(function (data) {
        ...
        $scope.onSelectSubHead = function (selection) {
            ...
            $scope.headWiseMonthlyYearlyDataShow($scope.subHeadSearch);
        }
        ...
        $scope.editExpense = function (i) {
            ...
            $scope.amount = parseFloat(i.credit);
            $scope.vouchar_date = i.vouchar_date;

            $scope.idSelectedRow = i.income_id;
            $scope.ex_id_edit = i.income_id;
        ...
        $http.get("/accounts/income/api/get-income-source-wise-report-data")
            .then(function (data) {
        ...
        $scope.updateExpense = function (form) {
            ...
            if (form.$valid) {
                ...
                $http.put("/accounts/income/api/update-income-data/" + $scope.ex_id_edit, data)
                    .then(function (r) {
                        console.log(r);
        ...
        $scope.deleteExpenseConfirm = function (i) {
            console.log(i);
            $scope.ex_id_delete = i.income_id;
        ...
        $scope.deleteExpenditure = function () {
            ...
            $http.get("/accounts/income/api/delete-income-data/" + $scope.ex_id_delete)
                .then(function (r) {
                    console.log(r);
