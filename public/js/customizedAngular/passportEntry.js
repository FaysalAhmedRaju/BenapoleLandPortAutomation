angular.module('passportEntryApp',['angularUtils.directives.dirPagination','customServiceModule'])
	.controller('passportEntryController', function($scope, $http,enterKeyService){

        $scope.showPassportDetails = false;
        $scope.showPassportDetailsSearch = false;
        $scope.visaButton = true;
        $scope.visaExit = true;
        $scope.buttonpassport =true;
		$scope.save = function() {
			...
			//console.log(data);
			$http.post("/passport/api/save-passport-entry-data", data)
				.then(function(data){
        ...
        enterKeyService.enterKey('#passportEntry input ,#passportEntry button')
        enterKeyService.enterKey('#passportForm input ,#passportForm button')
        enterKeyService.enterKey('#visaform input ,#visaform button')
...
$scope.getPassportVisaDetails = function (v_passport_no) {
    ...
    $http.post("/passport/api/all-visa-information-details",data)
        .then(function (data) {
		...
		$scope.saveVisa = function () {
            ...
            $http.post("/passport/api/save-visa-information-data",data)
                .then(function (data) {
		...
		$scope.passportInfo = function(passport_no) {
			...
			$http.post("/passport/api/get-passport-details-information", data)
				.then(function(data){
 ...
 if(data.data.length >=1) {
     ...
     $scope.date_of_expired = null;
		...
		$scope.SearchPassNOEntryExit = function (PassportNo) {
            ...
            $http.post("/passport/api/search-passport-entry-exit-data",data)
                .then(function (data) {
...
$scope.showPassportDetails = true;
                    $scope.passportID = data.data[0].p_id;
                    $scope.passport_no = data.data[0].passport_no;
        ...
        $scope.visaDetailsofPassport = function (passport_no) {
            ...
            $http.post("/passport/api/get-all-exit-entry-data-details",data)
                .then(function (data) {
        ...
        $scope.saveEntryExit = function () {
            ...
            $http.post("/passport/api/save-visa-entry-exit-data", data)
                .then(function(data){
                    ...
                    $scope.visaDetailsofPassport($scope.passport_no)
		...
		$scope.visaDetails = function (text) {
            ...
            console.log(data)
			console.log(text)
            $http.post("/passport/api/visa-information-details",data)
                .then(function (data) {
