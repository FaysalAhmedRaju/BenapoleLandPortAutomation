...
class CnfPanelController extends Controller
{
    ...
    public function welcome() {
        ...
        $name = Auth::user()->name;
        $todaysTruckTotal=DB::select('SELECT COUNT(id) total_truck_entry FROM truck_entry_regs WHERE DATE(truckentry_datetime)=?',[$currentDate]);
        $todaysManifestTotal=DB::select('SELECT COUNT(DISTINCT manf_id) total_manifest FROM truck_entry_regs WHERE DATE(truckentry_datetime)=?',[$currentDate]);

        $todaysManifestByUser=DB::select('SELECT COUNT(id) total_manifest_by_user FROM manifests WHERE manifest_posted_by=? AND  DATE(manifest_created_time) =? ',[$name,$currentDate]);

        $todaysTruckByUser=DB::select('SELECT COUNT(id) total_truck_by_user FROM truck_entry_regs WHERE created_by=? AND  DATE(truckentry_datetime) =? ',[$name,$currentDate]);



        return view('default.cnf.welcome',compact('todaysTruckTotal','todaysManifestTotal','todaysManifestByUser','todaysTruckByUser'));

    }

    public function CnFEntryForm()
    {
        return view('default.cnf.cnfEntryFormView');
    }

    public  function  cnfManifestWiseReportView()
    {
        return view('cnfManifestReport.cnfManifestReportView');
    }

    public  function  cnfExpoter()
    {
        return view('cnfManifestReport.cnfExpoterView');
    }

    public  function vatsDetailsData()
    {
    ...
    public function cnfGoodsDetailsData()
    {
    ...
    public function searchManifestData(Request $r) {
...
//            ->leftjoin('vatregs AS v','manifests.vat_id' , '=','v.BIN' )
            ->select(
...
//                'truck_entry_regs.gweight as t_gweight',
    ...
    public function getCnfVatDetails(Request $req)
    {
        $vat = DB::table('vatregs')
            ->where('BIN',$req->BIN)
    ...
    public function savePostingData(Request $req)
    {
...
//        return 'Successfully Updated';
    ...
    public function updateManifestPostingData(Request $req)
    {
        DB::table('manifests')
            ->where('manifest', $req->m_manifest)
            ->update(
...
//                    'goods_id' => $req->m_good_id,
    ...
    public function deleteTruckEntryData($i)
    {
...
//           return;
    ...
    public function getTodaysCnfManifestPostingReport() {
...
//                'vatregs.ADD1'
            ...
            ->get();
        $pdf = PDF::loadView('default.cnf.todaysCnfPostingPdf',[
            'mainData' => $mainData,
...
//            )
//            ->get();
//        $pdf = PDF::loadView('default.cnf.manifestReportForCnfPDF',[
...
//    }
    ...
    public function cnfManifestReport(/*$manifest,$truck*/ Request $r) {

        //$totalFigure = (string)$manifest."/".(string)$truck;
        $totalFigure = $r->ManifestNo;
        $todayWithTime = date('Y-m-d h:i:s a');
...
//            ->get();
    ...
    public function cnfImporterWiseReport() {
        return view('default.cnf.ImporterWiseReport');
    }

//    public function getImporterWiseReport(Request $r) {
//        //return $r->importerNo;
//        $todayWithTime = date('Y-m-d h:i:s a');
...
//        $pdf = PDF::loadView('default.cnf.ImporterWisePDFReport',[
//            'todayWithTime' => $todayWithTime,
...
//    }
    ...
    public function cnfCargoWiseReport() {
        return view('default.cnf.CargoWiseReport');
    }


    public function getCargoWiseReport(Request $r) {
        ...
        $pdf = PDF::loadView('default.cnf.CargoWisePDFReport',[
            'todayWithTime' => $todayWithTime,
    ...
    public function cnfDateWiseReport() {
        return view('default.cnf.DateWiseReport');
    }

    public function getDateWiseReport(Request $r) {
        ...
        $pdf = PDF::loadView('default.cnf.DateWisePDFReport',[
            'todayWithTime' => $todayWithTime,
